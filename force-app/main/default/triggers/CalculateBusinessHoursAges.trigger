...
trigger CalculateBusinessHoursAges on Case (before insert, before update) {
    if (Trigger.isInsert) {
        CaseBusinessHoursHandler.handleBeforeInsert(Trigger.new);
    }

    if (Trigger.isUpdate) {
        CaseBusinessHoursHandler.handleBeforeUpdate(Trigger.new, Trigger.oldMap);
    }
}